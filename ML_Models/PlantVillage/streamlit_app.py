...
# Replace with your IP Webcam URL (make sure it's the /shot.jpg endpoint)
# Example: http://<phone-ip>:8080/shot.jpg
IP_WEBCAM_URL = "http://192.168.137.62:8080/shot.jpg"  
...
# ---------------- STREAMLIT APP ----------------
st.title("Plant Disease Detection - IP Webcam")

st.markdown(
    f"""
    This app connects to your phone's **IP Webcam** feed.  
    Make sure you have the IP Webcam app running and replace the URL correctly.  
    Current URL: `{IP_WEBCAM_URL}`
    """
)

if st.button("Capture from Phone Camera"):
    try:
        # Grab image from IP Webcam
        img_resp = requests.get(IP_WEBCAM_URL, timeout=5)
        ...
        # Show captured frame
        st.image(frame_rgb, caption="Captured from Phone Camera", use_container_width=True)
        ...
        st.success(f"Prediction: Class {predicted.item()}")

    except Exception as e:
        st.error(f"Error capturing from IP Webcam: {e}")
